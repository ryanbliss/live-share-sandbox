...
const SandpackEditor: FC<ISandpackEditorProps> = (props) => {
    const { pages, codePageFiles, codePageFilesRef, activeFileRef, codemirrorInstance } = props
    ...
    const onPostDocumentChange = useCallback((fromA: number, toA: number, fromB: number, toB: number, inserted: any) => {
    ...
    const debouncedPostDocumentChange = useCallback(debounce(onPostDocumentChange, 0), [onPostDocumentChange, activePage]);
