...
import { INtpTimeInfo } from "@codeboxlive/hub-interfaces";
...
export class SharedClock implements ITimestampProvider {
  ...
  private _getNptTime: () => Promise<INtpTimeInfo>;

  constructor(getNptTime: () => Promise<INtpTimeInfo>) {
    this._getNptTime = getNptTime;
  }
  ...
  private async getSessionTimeOffset(): Promise<IServerTimeOffset> {
    ...
    const serverTime = await this._getNptTime();
